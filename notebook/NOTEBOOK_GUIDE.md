...
| `sps_bound_kaggle.ipynb` | Minimal SPS width probe: frozen CNO (`submission_v6`, same preds as v3) + fixed `pred ± bound_frac*\|pred\|` bands every step; sweeps widths with official SPS (§4 numpy == §5 `ttt_step` path), compares v4 adaptive (§5b), v5 relative-q90 + v7 EMA alpha-sweep (§5c), v8 speed-conditioned q90 (§5d), v9 1-step SGD + v10 adapt/calibrated (§5e) + v11 test-time LoRA (§5f, full subscore rows via §4b helper), `local_eval` sanity, packs one zip per width/alpha via `--set`. §7 maps signed spatial bias (over/under) + bias-vs-speed bins. | `VARIANT='submission_v6'`, `WIDTHS=[0.05,0.10,0.20,0.30,0.50]`, `ALPHAS=[0.10,0.30,0.50,0.70,1.00]`, `N_FILES=30`, `N_FRAMES=200` | `/kaggle/working/real30/` (staged), `dist/submission_v6_w*.zip`, `dist/submission_v4_cno.zip`, `dist/submission_v7_a*.zip` |
