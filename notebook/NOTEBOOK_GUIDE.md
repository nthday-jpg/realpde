...
| `eval_kaggle.ipynb` | Editor-first eval runner: smoke-test every variant via `local_eval.py`, stage real-30 + score CNO variants on GPU, SPS symmetry probe (v4 symmetric q90 vs asymmetric [q10,q90] in one run via `local_eval.sps_component_breakdown`), pack Codabench zips. Replaces `sps_bound_kaggle.ipynb` (removed 2026-09-19; its sweep logic now lives in `local_eval`). | `VARIANT='submission_v4'` (real-30 cell), `N_FILES=30`, `N_FRAMES=200`, `SEED=42` | `local_eval` logs + `/kaggle/working/real30/`, `dist/*.zip` |
